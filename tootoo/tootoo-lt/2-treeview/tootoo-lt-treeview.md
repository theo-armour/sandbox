...
1. Query `details[data-folder-path="{folderPath}"]`
...
4. Fetch tree from API
5. renderTree(state.tree) — batched
6. After all batches: auto-open README, show Expand All button
7. Set up filter input listener (debounced)
8. Set up keyboard navigation listener
9. Set up `/` shortcut
```
