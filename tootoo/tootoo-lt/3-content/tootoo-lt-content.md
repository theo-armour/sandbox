...
Start from the attached `tootoo-lt-treeview.html`. Add the content viewer external dependencies (`<script>`/`<link>` tags in `<head>`), CSS into the existing `<style>` block, and JS into the existing `<script>` block. Replace the placeholder `selectFile()` function with the full file-loading implementation. Save the combined result as a **new file** called `tootoo-lt-content.html`.
...
- `escapeHTML()` utility
- `getAuthHeaders()` — returns headers object with optional `Authorization`
- `updateRateLimit(response)` — reads rate-limit headers and updates `#rateBadge`
- `mainAbortController` — shared `AbortController` for canceling in-flight fetches
- `selectFile(path)` — placeholder that currently just shows "Selected: {path}" (to be replaced)
...
Extend the existing `applyDark()` to also call `setHljsTheme(on)`:
...
5. Call `loadTree()` to re-fetch and re-render
...
6. Check file text cache (for text-based files)
7. Fetch content (text or blob depending on type)
8. Render based on file type
9. Cache text content
10. Update hash (unless `skipHash` parameter is true)
11. Save to `lastFilePath` in localStorage
...
Extend the existing init to:

```
1. (existing) Restore sidebar width, dark mode, font size
2. (existing) Copy CONFIG → state, update header
3. Restore file text cache from localStorage
4. (existing) loadTree()
5. After tree loads:
   a. If URL hash present → handleHashChange() → open that file
   b. Else if lastFilePath in localStorage → open that file
   c. Else → auto-open README (already handled by treeview)
```
...
- `escapeHTML()` on all user-supplied strings before innerHTML insertion
- In `onclick` attributes, also replace `'` with `&#39;` after escaping to prevent attribute breakout
